...
// 목록에 바로 뜨는 기본 건조물들. 목재만 있으면 되고, 전부 격자 한 칸을
// 통째로 막는 장애물이다. 채굴기는 10초마다 광석을 하나씩 캐내고, 용광로는
// 그 광석을 철로 정련한다 (실제 처리는 tickZombieEconomy, server.js).
...
// 강화대에서 공격력을 올리는 값. n번째 구매(0부터 시작)는 5*(n+1) 코인 --
// 1번째 5, 2번째 10, 3번째 15... 처럼 살 때마다 5코인씩 비싸진다.
// 이 강화는 방(room)에 붙는 값이라 그 판이 끝나면(방이 사라지면) 함께 사라진다.
function zombieAttackUpgradeCost(level) {
    return 5 * (level + 1);
}
