...
// 강화대에서 파는 세 가지(공격력/터렛 공격력/울타리 체력) 전부 같은 값을
// 쓴다. n번째 구매(0부터 시작)는 5*(n+1) 코인 -- 1번째 5, 2번째 10, 3번째
// 15... 처럼 살 때마다 5코인씩 비싸진다. 이 강화들은 방(room)에 붙는
// 값이라 그 판이 끝나면(방이 사라지면) 함께 사라진다.
function zombieUpgradeCost(level) {
    return 5 * (level + 1);
}
const ZOMBIE_ATK_UPGRADE_AMOUNT = 1; // 공격력/터렛 공격력 모두 강화 1번당 +1
