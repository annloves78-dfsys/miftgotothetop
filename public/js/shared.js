...
// ==================== 캐릭터 레벨(성장던전) ====================
// 캐릭터별 영구 레벨업. 성장던전에서 번 EXP는 캐릭터별로 안 나뉘고 계정
// 전체가 같이 쓰는 공용 재화(gameData.sharedExp, storage.js)로 쌓이기만
// 하고, 레벨은 저절로 오르지 않는다 -- 캐릭터 상세 화면의 "레벨업" 버튼을
// 직접 눌러야 그 시점의 다음 레벨 비용(charLevelExpToNext)만큼 sharedExp에서
// 깎고 charLevels[charType]을 1 올린다. 레벨 자체는 캐릭터마다 따로
// charLevels에 그대로 저장해 두고(파생값이 아니다), 오르는 데 쓰는 재화만
// 공용이다.
...
// 대전(Arena) 모드 전용 전투 보정: PvE 스탯을 그대로 쓰지 않고 이 배율을
// 곱한다. overrides에 없는 charType은 default를 쓴다. 처음엔 전부
// 1.0(무보정)으로 시작 -- 실제 수치는 유누가 나중에 확정해서 준다.
const ARENA_BALANCE = {
    default: { health: 1, attackDamage: 1 },
    overrides: {}
};

// 대전모드(기지전) 5인 편성: 캐릭터 5명을 광산(1~2)/전방(1~3)/원거리(1~2)로
// 나눠 배치한다(합은 항상 5). 광산은 배치한 캐릭터의 등급에 따라 광석이
// 자동으로 쌓이고(간격), 죽은 캐릭터는 이 광석으로 부활시킨다(비용) --
// 두 표 다 GRADE_ORDER 문자열을 그대로 키로 쓴다.
const ARENA_UNIT_ROLES = ['mine', 'front', 'ranged'];
const ARENA_ROLE_MIN = { mine: 1, front: 1, ranged: 1 };
const ARENA_ROLE_MAX = { mine: 2, front: 3, ranged: 2 };
const ARENA_LINEUP_SIZE = 5;
const ARENA_BASE_RADIUS = 70; // 기지 하나의 반지름(월드 좌표) -- 서버(사거리 판정)와 클라이언트(그리기)가 같은 값을 써야 한다
const ARENA_MINE_INTERVAL_MS = {
    일반: 10000, 희귀: 10000,
    에픽: 8000, 레전더리: 8000,
    에이션트: 4000, 비스트: 4000,
    게스트: 2000
};
const ARENA_REVIVE_COST = {
    일반: 10, 희귀: 10,
    에픽: 20, 레전더리: 20,
    에이션트: 50, 비스트: 50,
    게스트: 100
