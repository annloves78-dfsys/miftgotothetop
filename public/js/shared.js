...
// 위 표만으로는 보스가 너무 물러서, 모든 레벨에 체력 300을 똑같이 더한다.
// 레벨마다 쌓이는 게 아니라 각 레벨 값에 300 한 번씩만 붙는다
// (1레벨 +70 -> +370, 10레벨 +300 -> +600).
const AWAKEN_BOSS_EXTRA_HEALTH = 300;
function awakenLevelHealthBonus(level) {
    const stats = awakenLevelStats(level);
    if (!stats) return 0;
    return (stats.health || 0) + AWAKEN_BOSS_EXTRA_HEALTH;
}
function awakenBossMaxHp(charType, level) {
    const base = CHARACTERS[charType];
    if (!base || !awakenLevelStats(level)) return null;
    return base.health + awakenLevelHealthBonus(level);
}
